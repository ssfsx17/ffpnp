...
Ranged attacks have full effectiveness on all targets.
...
Then, each combatant has passive AP gain. This gain is equal to Initiative * 0.01, rounded down (i.e. drop the tens and ones places). It can be inferred that the Initiative result must be at least 100 in order to have any passive AP gain.
...
Roll 1d100 + Accuracy. If the result is higher than the enemy's Evasion, then the attack hits. If the result is 100 higher than the enemy's Evasion, then it is a critical hit for double damage. If the attack hits, then the weapon damage is reduced by the target's Defense. The final damage is subtracted from the target's current Health Points. The target's Health Points cannot go below zero.
...
Balloon - Unable to act. Evasion is zero. Balloon status is ended by physical damage.

Berserk - Only able to do Attack actions. Physical damage output is doubled, but Evasion and Defense are zero.

Blind - Accuracy and Magic Accuracy are zero.
...
Broken Armor - Defense is zero. If this status happens to a player character, their body equipment is also nullified.

Broken Helmet - Magic Defense is zero. If this status happens to a player character, their head equipment is also nullified.
...
Float - Take no Earth damage. Ignore floor & tripwire traps.
...
Immobilize - Evasion and Magic Evasion are zero.
...
Old - Each attribute is decreased by 5 per round. When the status ends, the attributes are restored to normal.
...
Poison - Lose an amount of HP per round equal to Level. Magic Defense is not applied. Also causes damage every minute outside of combat.
...
Tranq - Accuracy is doubled, but Evasion is zero.

Trouble - When you take physical damage, record half of it. This amount of physical damage is done to each of your party members.

Vanish - Evasion is infinite, but Magic Evasion is zero.

Virus - Poison, except that it also prevents healing.
